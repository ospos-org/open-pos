...
export default function App() {
	...
	const fetch_cookie = async () => {
		fetch('http://127.0.0.1:8000/employee/auth/3ff66422-4d11-4db3-8bf3-073e8cc0b115', {
			method: "POST",
			body: JSON.stringify({
				pass: "1232"
			}),
			...
			fetch('http://127.0.0.1:8000/employee/3ff66422-4d11-4db3-8bf3-073e8cc0b115', {
				method: "GET",
				credentials: "include",
				redirect: "follow"
			}).then(async k => {
			...
			<div className="flex flex-row h-[calc(100vh-18px)] flex-shrink-0">
				...
				<div className="bg-gray-900 flex flex-col p-4 h-full justify-between items-center">
					<div className="flex flex-col h-full gap-12 items-center">
						...
						{
							page == 0 ?
							<Image className="svg" width={`${ICON_SIZE}`} height={`${ICON_SIZE}`} src="/icons/shopping-bag-01-filled.svg" alt="" style={{ filter: "invert(100%) sepia(0%) saturate(7484%) hue-rotate(116deg) brightness(96%) contrast(101%)" }} onClick={() => setPage(0)}></Image>
							:	
							<Image width={`${ICON_SIZE}`} height={`${ICON_SIZE}`} src="/icons/shopping-bag-01.svg" style={{ filter: "invert(61%) sepia(16%) saturate(286%) hue-rotate(175deg) brightness(90%) contrast(90%)" }} alt={''} onClick={() => setPage(0)}></Image>
						}
