...
function determineProductDirectionality(transactionType: TransactionType): number {
...
const totalNetQuantityOfProductTransactedOutFromCartAtom = atom(
    ...
    (get, set, value: ContextualProductPurchase) => {
        return get(ordersAtom).reduce((prior, current) =>
            prior + current.products.reduce((priorProduct, currentProduct) =>
                priorProduct + (
                    currentProduct.variant_information.barcode === value.variant_information.barcode
                        // We need the net value transfer.
                    ? (determineProductDirectionality(currentProduct.transaction_type) * currentProduct.quantity)
                    : 0
                ),
                0
            ),
        ...
        return get(ordersAtom).reduce((prior, current) =>
                prior + current.products.reduce((priorProduct, currentProduct) =>
                        priorProduct + (
                            currentProduct.variant_information.barcode === value.variant_information.barcode
                                // We need the net value transfer.
                                ? ((currentProduct.transaction_type === "Out") ? currentProduct.quantity : 0)
                                : 0
                        ),
                    0
                ),
