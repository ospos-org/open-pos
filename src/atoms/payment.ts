...
const currentPaidAmountAtom = atom<number>((get) =>
	get(paymentHistoryAtom).reduce(
		(previous, modification) => previous + modification.quantity,
		0,
	),
);
...
const priceAtom = atom(
	(get) => {
		...
		const non_discounted_sub_total = applied_promotions.reduce(
			(p, c) =>
				p +
				applyDiscount(
					get(c).products.reduce(
						(prev, curr) =>
							prev + curr.variant_information.retail_price * curr.quantity,
						0,
					),
					get(c).discount,
				),
		...
		const sub_total = applied_promotions.reduce(
			(p, c) =>
				p +
				applyDiscount(
					get(c).products.reduce(
						(prev, curr) =>
							prev +
							(curr.variant_information.retail_price * curr.quantity -
								applyDiscountsConsiderateOfQuantity(
									curr.quantity,
									curr.discount,
									curr.variant_information.retail_price,
									get(aCustomerActiveAtom),
								)),
						0,
					),
					get(c).discount,
			...
			(p, c) =>
				(p += applyDiscount(
					get(c).products.reduce(
						(prev, curr) =>
							prev +
							(curr.variant_information.retail_price * 1.15 * curr.quantity -
								applyDiscountsConsiderateOfQuantity(
									curr.quantity,
									curr.discount,
		...
		const tax = total - sub_total;
		const payable = total - get(currentPaidAmountAtom);
...
const generateIntentAtom = atom(undefined, (get, set) => {
