...
const searchInputRefAtom = atom<RefObject<HTMLInputElement>>(
...
const searchTypeHandlerAtom = atom(
	...
	(get, set, value: keyof SearchResults) => {
		...
		if (get(searchInputRefAtom).current?.value)
			get(searchInputRefAtom).current.value = "";
...
const querySearchTerm = atom(undefined, async (get, set) => {
	...
	const performQuery = async () => {
		switch (searchType) {
			...
			case "products":
				return await openStockClient.product.searchWithAssociatedPromotions(
					searchTerm,
				);
			case "transactions":
