...
const ordersAtom = atom(
	...
	(get, set, value: ContextualOrder[]) => {
		...
		const flat_products = get(ordersAtomsAtom)
			.map((k) => get(k).products)
			.flatMap((k) => k);

		const optimal_products = determineOptimalPromotionPathway(flat_products);
		const optimal_queue = optimal_products.filter(
			(b) =>
				b.chosen_promotion?.promotion != null && b.chosen_promotion != null,
		);

		const applied_promotions = get(ordersAtomsAtom).map((b) => {
			return {
				...get(b),
				products: get(b).products.map((k) => {
					const dim = [];
					...
					for (let i = 0; i < k.quantity; i++) {
						...
						if (
							exists !== -1 &&
							optimal_queue[exists].chosen_promotion != null &&
							optimal_queue[exists].chosen_promotion?.promotion != null
						) {
							// Apply discount
							const di = {
								source: "promotion",
					...
					return {
						...k,
						discount: [
							...k.discount.filter((b) => b.source !== "promotion"),
							...dim,
						],
					};
				}),
