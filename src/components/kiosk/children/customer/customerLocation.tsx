...
import {createRef, Dispatch, useCallback, useEffect, useMemo, useState} from "react";
import {debounce} from "lodash";
import {SetStateAction} from "jotai";

import {openStockClient} from "~/query/client";
import {Address, ContactInformation, Customer, CustomerInput} from "@/generated/stock/Api";
...
function CustomerLocation({ workingCustomer, updateCustomer }: CustomerLocationProps) {
    ...
    const verifiedAddress = useMemo(() => (
        <div className="flex flex-row items-start gap-4 px-2 py-4">
            ...
            <div className="text-white">
                <p className="font-semibold">{workingCustomer?.contact.address.street}</p>
                <p>{workingCustomer?.contact.address.street2}</p>
                <p className="text-gray-400">{workingCustomer?.contact.address.city} {workingCustomer?.contact.address.po_code}</p>
                <p className="text-gray-400">{workingCustomer?.contact.address.country}</p>
            </div>
        </div>
    ), [workingCustomer])

    const selectSuggestion = useCallback((suggestion: Address) => {
        setSearching(false);
    ...
    const searchElement = useMemo(() => {
        ...
        else if (searching) return suggestions.length > 0 ? suggestionsList : noAddressFound

        return workingCustomer?.contact.address.street === "" ? noAddress : verifiedAddress
    }, [loading, noAddress, noAddressFound, searching, suggestions.length, suggestionsList, verifiedAddress, workingCustomer?.contact.address.street])
