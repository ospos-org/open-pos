...
import {GeneratedOrder, SelectedItem} from "@components/kiosk/children/foreign/common/generated";
...
export function ProductPullStore({ selectedItemsAtom, generatedOrderAtom, order }: ProductPullStoreProps) {
    ...
    const alternateStoreSelectorOpen = useMemo(() =>
        selectedItems.find(b => (b.item_id == order.item?.id && b.store_id == order.store))?.selected,
        [order.item?.id, order.store, selectedItems]
    )

    const setSelected = useCallback(() => {
        setSelectedItems(selectedItems.map(b =>
    ...
    const chooseAlternateStore = useCallback((store: Stock) => {
        setGeneratedOrder(generatedOrder.map(b =>
    ...
    }, [generatedOrder, order.item?.id, order.store, selectedItems, setGeneratedOrder, setSelectedItems])

    return (
        <div
            className={
                `relative inline-block ${
                    Boolean(selectedItems.find(b => 
                        (b.item_id == order.item?.id && b.store_id == order.store)
                    )?.selected) && "z-50" 
                }`
            }
        >
            ...
            >
                {currentStore.store_lut?.length > 0
                    ? currentStore.store_lut?.find((b: Store) => order.store == b.id)?.code
                    : "-"
                }
            </p>
            ...
            <div
                className={
                    `${Boolean(!alternateStoreSelectorOpen) && "hidden"} absolute flex flex-col items-center 
