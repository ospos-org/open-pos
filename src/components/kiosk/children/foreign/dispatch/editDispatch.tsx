...
export default function EditDispatch({ callback }: EditDispatchProps) {
    ...
    const setSearch = useCallback((address: Address) => {
        setSearching(false);
    ...
    return (
        <div className="flex flex-col gap-8 flex-1 max-h-fit overflow-hidden">
            ...
            <div className="flex flex-col w-full gap-2 rounded-md flex-1">
                ...
                <div className="h-full">
                    <div className="flex flex-col gap-1">
                        <div
                            className="flex flex-row items-center p-4 rounded-t-sm bg-gray-700 gap-4 border-2 border-gray-700"
                        >
                            <input
                                autoComplete="off"
                                ref={input_ref}
            ...
            >
                <p className="text-white font-semibold">{loading && !searching ? "Saving..." : "Save"}</p>
            </div>
        </div>
    )
}