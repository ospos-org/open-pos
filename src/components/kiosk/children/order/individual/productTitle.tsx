...
export function ProductTitle({ currentOrder, product, quantityHere, totalStock }: ProductTitleProps) {
    ...
    }, [currentOrder.products, product, quantityHere?.quantity.quantity_sellable])

    return (
        <div
            className="flex-1 cursor-pointer"
            onClick={focusProduct}
        >
            ...
            {Boolean(
                ...
                <p className="text-xs text-red-400">
                    ...
                    , {(totalStock - (quantityHere?.quantity.quantity_sellable ?? 0)) ?? 0} in other stores
                </p>
            )}
        </div>
    )
