...
const GRAY_FILTER =
	"invert(100%) sepia(17%) saturate(7473%) hue-rotate(290deg) brightness(122%) contrast(114%)";
...
export function SavedTransactionItem({ transaction }: TransactionItemProps) {
	...
	useEffect(() => {
		if (transaction?.customer.customer_type !== "Store") {
			...
			openStockClient.store
				.getByCode(transaction?.customer.customer_id)
				.then((data) => data.ok && setCustomer(data.data));
		}
	}, [transaction]);
	...
	const openTransaction = useCallback(async () => {
		...
		const updated_orders: ContextualOrder[] = await Promise.all(
			transaction.products.map(async (k) => {
				const new_products = k.products.map(async (b) => {
					const response =
						await openStockClient.product.getWithAssociatedPromotions(
							parseInt(b.product_sku),
						);
							...
							discount: [
								{
									...
									source:
										fromDbDiscount(b.discount) ===
										fromDbDiscount(variant?.loyalty_discount ?? { Absolute: 0 })
											? "loyalty"
											: "user",
		...
		<div className="flex flex-col gap-[4px] items-center p-4 text-white border-r-2 border-gray-600">
			<div className="flex-1 w-full px-2">
				<div className="flex flex-row gap-2 justify-between w-full flex-1 items-center">
					<h3 className="text-gray-400 flex-1 w-full flex-nowrap whitespace-nowrap">
						{customer?.name}
					</h3>
					...
					<div className="flex flex-row items-center gap-[0.2rem] flex-1 w-full">
						<p className="text-sm whitespace-nowrap">
							{transaction.products.reduce((p, c) => p + c.products.length, 0)}{" "}
							item
							{transaction.products.reduce((p, c) => p + c.products.length, 0) >
							1
								? "s"
			...
			<div className="flex flex-row items-center w-full gap-2 rounded-full px-1">
				<div className="flex flex-row items-center gap-2 bg-gray-600 px-[4px] pr-[8px] py-[2px] rounded-full h-[26px]">
					...
					<p className="text-sm">
						...
						{moment(
							new Date(
								new Date(transaction.order_date).getTime() + 3_600_000,
							).getTime(),
						).fromNow()}
					</p>
				</div>
