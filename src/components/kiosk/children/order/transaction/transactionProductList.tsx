...
export default function TransactionProductList() {
    ...
    const [
        selectedItems,
        setSelectedItems
    ] = useAtom(transactionViewState.selectedItems);

    const isMatchingItem = useCallback((thisPurchase: ProductPurchase) => {
        return selectedItems.findIndex(
            (elem) => elem.product_id === thisPurchase.id
        )
    }, [selectedItems])

    const selectProduct = useCallback((thisPurchase: ProductPurchase) => {
        if (isMatchingItem(thisPurchase) !== -1) {
            setSelectedItems(selectedBefore =>
                selectedBefore.filter((element) =>
                    element.product_id !== thisPurchase.id
                )
            )
        } else {
    ...
    const modifyQuantity = useCallback((method: "SUBTRACT" | "ADD", thisPurchase: ProductPurchase) => {
        if (isMatchingItem(thisPurchase) === -1) {
            setSelectedItems([...selectedItems, {
                product_id: thisPurchase.id,
                quantity: thisPurchase.quantity
        ...
        const mapValues = selectedItems.map(
            (element) => {
                if (element.product_id === thisPurchase.id)
                    return {
                        ...element,
                        quantity: method === "SUBTRACT"
    ...
    return (
        <div className="flex flex-col gap-2">
            ...
            {activeTransaction?.products.map(k => {
                ...
                return (
                    <div key={`${k.id} ${k.product_code} ${k.quantity} ${k.product_variant_name}`}
                    ...
                    >
                        <div className="flex flex-row items-center gap-2 w-fit">
                            ...
                            <div
                                className={`
                                    ...
                                    ${matchingItem === -1 ? "bg-gray-600" : "bg-gray-100"}
                                `}
                            >
                                <p
                                ...
                                <p className="text-gray-200 px-2 bg-gray-800">
                                    {matchingItem === -1 ? k.quantity : selectedItems[matchingItem].quantity}
                                </p>
                        ...
                        <div className="flex flex-col items-center justify-center">
                            <p className="text-white font-semibold text-sm">
                                ${applyDiscount(k.product_cost, fromDbDiscount(k.discount)).toFixed(2)}
                            </p>
            ...
            >
                ...
                <p className="text-white font-semibold">
                    ${
                        activeTransaction?.products.reduce((prev, k) =>
                            prev + applyDiscount(k.product_cost * k.quantity, fromDbDiscount(k.discount)),
                            0
                        ).toFixed(2)
                    }
                </p>
