		...
		<div className="flex flex-row items-center gap-2">
			<div
				className={`text-white px-4 py-2 rounded-md ${
					selectedItems.length > 0 ? "bg-gray-600" : "bg-gray-800"
				...
				onClick={() => {
					...
					const products_being_returned = selectedItems
						.map((elem) => {
							const reference = activeTransaction?.products.find(
								(element) => element.id === elem.product_id,
							);
							if (!reference) return undefined;

							reference.quantity = elem.quantity;

							reference.instances = reference?.instances.filter(
								(_, index) => index < elem.quantity,
							);
							return reference;
						})
					...
					setKioskState(RESET);
					setPerfState({
						type: "continuative" as const,
						transaction_id: activeTransaction.id,
					});

					const conv_to_product_purchase = products_being_returned.map(
						async (product) => {
							const response =
								await openStockClient.product.getWithAssociatedPromotions(
									parseInt(product.product_sku),
								);

							if (!response.ok) return;

							const variant_info = response.data.product.variants.find(
								(element) => element.barcode === product.product_code,
							);
					...
					Promise.all(conv_to_product_purchase).then((fulfilled) => {
						const products = fulfilled.filter(
							(e) => e !== undefined,
						) as ContextualProductPurchase[];
