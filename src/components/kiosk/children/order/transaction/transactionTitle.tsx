...
export default function TransactionTitle() {
	...
	const [customer, setCustomer] = useAtom(
		transactionViewState.attachedCustomer,
	);
	...
	useEffect(() => {
		...
		if (transaction?.item.customer.customer_type !== "Store") {
			...
			openStockClient.store
				.getByCode(transaction?.item.customer.customer_id)
				.then((data) => data.ok && setCustomer(data.data));
		}
			...
			<div className="flex flex-row items-center justify-between w-full">
				<div className="flex flex-col">
					...
					<p className="text-lg font-semibold text-white">
						...
						{capitalize(activeTransaction?.order_type)}
					</p>
				</div>
