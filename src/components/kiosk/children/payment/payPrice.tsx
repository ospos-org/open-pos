...
export function PayPrice() {
	...
	const remainingDue = useMemo(() => {
		const totalPaid = kioskState.payment.reduce(
			(prev, curr) => prev + curr.amount.quantity,
			0,
		);
	...
	const submitPayment = useCallback(() => {
		...
		if (targetValue == "") {
			...
			const p = parseFloat(targetValue);
	...
	return (
		<div className="self-center flex flex-col items-center">
			...
			{Boolean(remainingDue > 0) && (
				<p className="text-gray-500">${remainingDue} remains</p>
			)}
			...
			>
				<Image
					src="/icons/coins-stacked-03.svg"
					height={20}
					width={20}
					alt=""
