...
export function PaymentMethod() {
	...
	useEffect(() => {
		...
		openStockClient.transaction.create(generateTransaction).then((data) => {
			...
			b.products.some((p) =>
				p.product.variants.some((n) => {
					if (p.product_code === n.barcode) {
						...
						const stockLevel = n.stock.reduce(
							(prev, curr) =>
								prev +
								(curr.store.store_id === storeId
									? curr.quantity.quantity_sellable
									: 0),
							0,
						);
		...
		>
			<div
				className={`flex flex-col h-full ${
					showNegativeStockWarning ? "gap-14" : "gap-20"
				}`}
			>
