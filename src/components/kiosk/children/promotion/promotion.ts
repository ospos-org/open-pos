...
function formatPromotion(promo: Promotion) {
	if (promo.get.type === "solothis") {
		return `${promo.name} - Get ${
			parseDiscount(fromDbDiscount(promo.get.value)) + " off."
		}`;
	}
	...
	if (promo.get.type === "this") {
		val +=
			...
			promo.get.value[0] +
			", " +
			parseDiscount(fromDbDiscount(promo.get.value[1]));
		...
		val +=
			...
			promo.get.value[0] +
			", " +
			parseDiscount(fromDbDiscount(promo.get.value[1][1])) +
			" off.";
	}
