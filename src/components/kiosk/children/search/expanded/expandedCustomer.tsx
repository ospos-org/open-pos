...
export function ExpandedCustomer() {
	...
	const [activeCustomerTransactions, setActiveCustomerTransactions] = useState<
		Transaction[] | null
	>(null);

	const sortedTransactions = useMemo(
		() =>
			activeCustomerTransactions?.sort(
				(a, b) =>
	...
	useEffect(() => {
		if (inspectingCustomer) {
			openStockClient.customer
				.findRelatedTransactions(inspectingCustomer.id)
		...
		<div className="p-4 text-white flex flex-col gap-8 bg-opacity-50 rounded-sm">
			<div className="flex flex-1 flex-row items-start h-full justify-between gap-2">
				<div className="flex flex-col gap-2">
					...
					<p className="text-gray-400 whitespace-pre-wrap flex-wrap">
						{inspectingCustomer.contact.address.street}{" "}
						{inspectingCustomer.contact.address.street2},{" "}
						{inspectingCustomer.contact.address.city}{" "}
						{inspectingCustomer.contact.address.po_code},{" "}
						{inspectingCustomer.contact.address.country}
					</p>
					...
					<div className="flex 2xl:flex-row flex-col 2xl:items-center 2xl:gap-4 gap-2">
						...
						<div className="bg-gray-700 w-fit flex flex-row items-center gap-4 px-2 py-2 rounded-md">
							...
							<p className="text-gray-200 font-semibold">
								{(() => {
									const k = inspectingCustomer.contact.mobile.number.match(
										/^(\d{3})(\d{3})(\d{4})$/,
									);
									if (!k) return "";
									return `${k[1]} ${k[2]} ${k[3]}`;
													...
													<p>
														{(() => {
															switch (k.order_type) {
																...
																case "shipment":
																	return `${k.origin.contact.name} (${
																		k.origin.store_code
																	}) -> ${
																		k.destination?.store_code !== "000"
																			? k.destination?.store_code
																			: k.destination?.contact.address.street
																	} ${
																		k.destination?.store_code !== "000"
																			? k.destination?.contact.name
																			: k.destination?.contact.address.street2
																	}`;
																default:
																	return "";
															}
