import {CustomerWithTransactionsOut, ProductWPromotion, Transaction} from "@/generated/stock/Api";
...
export type ItemUnion = CustomerWithTransactionsOut | ProductWPromotion | Transaction
export type SearchUnion = CustomerSearch | ProductSearch | TransactionSearch

export function hashSearch(value: SearchUnion) {
    if (value.type === "products") return value.results.map(value => value.product.sku)
