	...
	return (
		<div
			...
			onClick={() => {
				...
				for (let i = 0; i < product.variants.length; i++) {
					const var_map = product.variants[i].variant_code.flatMap((k) => {
					...
					const vlist: StrictVariantCategory[] = var_map
		...
		>
			<div
				...
				style={{
					gridTemplateColumns:
						(windowSize?.width ?? 0) < 1536
							? (windowSize?.width ?? 0) < 640
								? "50px 1fr 0px 75px"
								: "50px minmax(180px, 1fr) 225px 100px"
							: "50px minmax(200px, 1fr) minmax(300px, 2fr) 250px 125px",
				}}
			>
				<Image
					height={50}
					...
					{product.variant_groups.map((variantGroup) => {
						return (
							<div
								...
								className="bg-gray-600 flex flex-row items-center py-1 px-2 rounded-md gap-2 max-h-fit"
							>
				...
				<div className="flex-1">
					{(() => {
						...
						const total_stock_in_store = product.variants
							.map((k) => {
								return k.stock
									.map((b) => {
										let total = 0;
						...
						return (
							<div className="hidden md:flex flex-row items-center gap-2">
								...
								<p className="text-gray-400">
									{total_stock - total_stock_in_store} in other stores
								</p>
							</div>
						);
					})()}
				...
				<div className="flex flex-row items-center px-2 font-medium">
					{(() => {
						const flat_map = product.variants.map((k) => k.retail_price);
