						...
						b.products.map((n) => {
							n?.instances?.map((l) => {
								if (
									...
									pairings.set(
										l.fulfillment_status?.pick_status,
										pairings.get(l.fulfillment_status?.pick_status) + 1,
									);
								}
							});
						});

						const mapped = Array.from(pairings, ([name, value]) => ({
							name,
							value,
						}));
						mapped.sort((a, b) => {
								...
								>
									<div className="flex flex-col gap-2">
										<div className="flex flex-row gap-2">
											{b.status.status?.type === "transit" ||
											b.status.status?.type === "instore" ||
											b.status.status.type === "fulfilled" ? (
												<p className="text-white font-mono font-bold px-2 bg-blue-600 rounded-md py-1">
													<Check color="white" size={18} />
												</p>
											) : completed === total_products ? (
												<p className="text-white font-mono font-bold px-2 bg-green-600 rounded-md">
													{completed}/{total_products}
												</p>
											) : (
												<p className="text-white font-mono font-bold px-2 bg-gray-700 rounded-md">
													{completed}/{total_products}
												</p>
											)}
											<p className="text-white font-semibold not-italic md:visible hidden">
												Products Picked
											</p>
										</div>
										...
										<div className="flex flex-row items-center gap-2">
											<div className="flex flex-row items-center justify-between gap-2">
												{mapped.map(
													(status: { name: PickStatus; value: number }) => {
														return (
															<div
																key={JSON.stringify(status)}
																className={
																	"flex flex-row items-center bg-gray-700 rounded-full max-h-4 pr-2 gap-1 justify-between w-full flex-1"
																}
															>
																{(() => {
																	switch (status.name) {
