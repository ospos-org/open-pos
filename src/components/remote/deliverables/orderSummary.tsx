...
export function OrderSummary() {
	...
	return (
		<div
			className="absolute pointer-events-none sm:relative flex flex-col h-full overflow-y-scroll"
					...
					{menuInformation ? (
						<div className="flex flex-col">
							...
							<p className="text-gray-400">
								{menuInformation?.description.substring(0, 40)}...
							</p>
						</div>
					) : (
						...
						<div className="flex flex-col gap-2">
							{menuState?.instances.map((k) => {
								return (
									<div
										key={JSON.stringify(k)}
										className="flex flex-row justify-between text-white pl-4 border-gray-800 bg-gray-900 border-2 w-full items-center p-2 rounded-lg"
									>
										<div className="flex flex-row items-center gap-2">
											...
											<p>
												{k.state.fulfillment_status?.pick_status.toString()}
											</p>
										</div>
										...
										<p
											...
											className="bg-gray-100 rounded-md text-gray-800 font-bold px-8"
										>
											-{">"}
										</p>
									</div>
			...
			{stateChange != null ? (
				<div className="absolute pointer-events-auto sm:relative overflow-y-scroll flex flex-col gap-4 z-50 bottom-0 sm:mb-0 mb-[40px] sm:h-full h-[440px] p-4 sm:w-full w-screen bg-black text-white h-80px sm:rounded-none rounded-t-md">
					<div className="flex flex-col">
						...
						<p>
							{stateChange.state.fulfillment_status?.pick_status.toString()}
						</p>
					</div>
						...
						>
							{stateChange.state.fulfillment_status?.pick_history.map((k) => {
								return (
									<div
										key={JSON.stringify(k)}
										className="grid flex-row items-center"
