...
export function SwitchViews() {
    ...
    const orderGroupViewCallback = useCallback((order: Order) => {
        setActiveOrder(order)
        setLowModeCartOn(!lowModeCartOn)
        setStateChange(null)
        setMenuState(null)
    }, [lowModeCartOn, setActiveOrder, setLowModeCartOn, setMenuState, setStateChange])
        ...
        <div className="flex flex-col md:p-4 p-6 w-full">
...

            {
                (() => {
                    switch(viewingMode) {
                        case 0:
                            return <OrderGroupView setActiveCallback={orderGroupViewCallback}/>
                        case 1:
