...
export const determineOptimalPromotionPathway = (
	...
	products.map((b) => {
		...
		const mapped: [Promotion, number][] = b.active_promotions
			.map((d) => {
	...
	while (product_queue.length > 0) {
		...
		while (true) {
			...
			if (
				["category", "any", "specific"].includes(optimal_promotion[0].get.type)
			) {
				...
				product_queue.map((ref) => {
					...
					if (val) {
						...
						if (
							...
							if (
								optimal_promotion &&
								optimal_promotion[0].get.type === "category"
							) {
								//console.log("Matching Category?", optimal_promotion[0].get);

								if (
									val.tags.includes(optimal_promotion[0].get.value?.[0] ?? "")
								) {
									external_source_id = val.id;
								}
							} else if (
								optimal_promotion &&
								optimal_promotion[0].get.type === "any"
							) {
								external_source_id = val.id;
							} else if (
								optimal_promotion &&
								optimal_promotion[0].get.type === "specific"
							) {
				...
				if (external_source_id == null) {
					...
					promotional_index += 1;
					if (promotional_index >= point.promotion_sim.length - 1)
						optimal_promotion = null;
		...
		if (
			optimal_promotion != null &&
			(point.utilized != null || point.is_joined)
		) {
			// && !point.is_joined
